...
class Email_temp_request extends CI_Controller
{
	...
	{		
		...
		if($type=='approved')
		{
			if(count($email_temp_id)>1)
			{	
				    $this->session->set_flashdata('success','Email template requests has been approved successfully');
			}
			else
			{
					$this->session->set_flashdata('success','Email template request has been approved successfully');
			}
		}
		else
		{
			if(count($email_temp_id)>1)
			{
				$this->session->set_flashdata('success','Email template requests has been rejected successfully');
			}
			else
			{
				$this->session->set_flashdata('success','Email template request has been rejected successfully');
			}
		}
	...
	public function getTemplateContent()
	{
			...
			echo $email_temp_content[0]->etr_description;	
	}
