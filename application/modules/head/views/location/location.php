			...
			<div class="col-md-12">
				<div class=" app-ticket-list">
					<div class="page-bar row">
						<div class="col-md-6">
							<h3 class="page-title"><?php getLocalkeyword('Location Management');?></h3>
							<ul class="page-breadcrumb ">
								<li><a href="<?php echo base_url()."head/dashboard ";?>"><?php getLocalkeyword('Home');?></a> <i class="fa fa-circle"></i></li>
								<li><span><?php getLocalkeyword('Location Management');?></span></li>
							</ul>
						</div>
						<div class="col-md-6">
								...
								<div id="addFestival" class="modal fade in" role="dialog">
									<div class="modal-dialog">
										...
										<div class="modal-content">
											<div class="modal-header"> <button type="button" class="close" data-dismiss="modal" onclick="resetLocationForm();">×</button>
												<h2 class="modal-title text-left"> <i class="fa fa-plus"></i> <?php getLocalkeyword('Add Country');?> </h2>
											</div>
											<div class="modal-body">
												<div class="portlet-body form">
													<form role="form" id="frmAdd" method="post" action="<?php echo base_url()."head/location/addCountry "?>">
														<div class="form-body">
															<div class="form-group"> <label class=""><?php getLocalkeyword('Country Name');?></label> <br> <input type="text" name="country" id="country" class="input-sm form-control " placeholder="Enter country name" required="required"> </div>
															<div class="modal-footer"> <button type="submit" name="submit" class="btn btn-primary">Save</button> <button type="button" data-dismiss="modal" class="btn btn-danger" onclick="resetLocationForm();">Cancel</button> </div>
														</div>
													</form>
												</div>
											</div>
									...
									<div class="modal-dialog">
										...
										<div class="modal-content">
											<div class="modal-header"> <button type="button" class="close" data-dismiss="modal">×</button>
												<h2 class="modal-title text-left"> <i class="fa fa-plus"></i> <?php getLocalkeyword('Edit Country');?></h2>
											</div>
											<div class="modal-body">
												<div class="portlet-body form">
													<form role="form" method="post" id="frmEdit" action="<?php echo base_url()."head/location/editCountry "?>">
														<div class="form-body">
															<div class="form-group"> <label class=""><?php getLocalkeyword('Country Name');?></label> <br> <input type="text" name="txtEditcountry" id="txtEditcountry" class="input-sm form-control " placeholder="Enter country name" required="required"> <input type="hidden" name="txtEditcountryid" id="txtEditcountryid" class="input-sm form-control "> </div>
															<div class="modal-footer"> <button type="submit" name="submit" class="btn btn-primary">Save</button> <button type="button" data-dismiss="modal" class="btn btn-danger">Cancel</button> </div>
														</div>
													</form>
												</div>
											</div>
                                                            ...
                                                            <div class="row">
                                                                ...
                                                                <div class="col-md-12">
                                                                    ...
                                                                    <div class="modal-footer"> 
                                                                        ...
                                                                        <a onclick="deleteCountry();" class="btn btn-primary"><?php getLocalkeyword('Yes');?></a>
								...
								<div class="col-md-12">
									<div class="table-toolbar">
										<div class="row">
                                            ...
                                            if(getAccessRights('mvbLocDelete'))
                                            {
                                            ?>
											...
											?> </div>
                                            <?php
                                             if (getAccessRights('mvbLocDownload'))
                                              {
                                                                                        ?> 
                                            <div class="btn-group" > <a  id="sample_editable_1_new hvr-float-shadow" data-toggle="modal" data-target="#otp1" onclick="getOTP();" class="btn btn-danger" ><i class="fa fa-cloud-download"></i>
                                            <?php getLocalkeyword('Download Country CSV');?></a> </div>
											<?php
                                                                                        }

											if (getAccessRights('mvbLocAdd'))
											{
												?>
														<div class="btn-group"> <a id="sample_editable_1_new hvr-float-shadow" data-toggle="modal" data-target="#addFestival" class="btn btn-primary"><?php getLocalkeyword('Add Country');?></a> </div>
														<?php
											}
											if (getAccessRights('mvbLocActivate') || getAccessRights('mvbLocDeactivate'))
											{
												?>
											...
											<?php
												...
												<ul class="dropdown-menu dropdown-menu-default">
												<?php
												if (getAccessRights('mvbLocActivate'))
												{
												?>
												...
												if (getAccessRights('mvbLocDeactivate'))
												{
													?>
										...
										<div class="portlet light">
											...
											for($i=0;$i < count($result);$i ++)
											{
												...
												?>
															<tr class="odd gradeX">
																<td><input name="chk<?php echo $j;?>" class="check_box" type="checkbox" data-ptag="sb1" id="chk<?php echo $j;?>" value="<?php echo $result[$i]->cntr_id_pk;?>"></td>
																<td>
																	<?php echo $j;?> </td>
																<td><a href="<?php echo base_url()."head/location/getallState/".base64_encode($result[$i]->cntr_id_pk);?>"><?php echo $result[$i]->cntr_name;?> </a> <input type="hidden" id="country_list<?php echo $j;?>" value="<?php echo $result[$i]->cntr_name; ?>"> <input type="hidden" id="country_id<?php echo $j;?>" value="<?php echo $result[$i]->cntr_id_pk; ?>"></td>
																<td>
																	<?php echo date("d-m-Y H:i:s", strtotime($result[$i]->cntr_createdDate));?> </td>
																<td>
																	<?php echo $result[$i]->created_by;?>
																</td>
																<td>
																	<?php echo date("d-m-Y H:i:s", strtotime($result[$i]->cntr_modifiedDate));?> </td>
																<td>
																	<?php  echo $result[$i]->modified_by;?>
																</td>
																<td>
																	<?php
												if ($result[$i]->cntr_status == 1)
												{
                        ...
                        <div class="modal-content" style="height: 150px;">
                            ...
                            <form id="otp_verify_form" name="otp_verify_form" method="POST" action="<?php echo base_url();?>head/location/verifyExport">
                                <div class="modal-body">
                                    ...
                                    <input type="hidden" class="form-control input-sm" id="hidden_type" name="hidden_type" value="country_module">	
                                    <a>
...
function get_record() {
	...
	for (var i = 1; i <= count; i++) {
		if ($('#chk' + i).is(':checked')) {
			country = $("#chk" + i).val();
			var txtCountry = $("#country_list" + i).val();
			var txtCountryid = $("#country_id" + i).val();
			$('#txtEditcountryid').val(txtCountryid);
			$('#txtEditcountry').val(txtCountry);
		}
	}
}
...
function chechCheckbox(id) {
	...
	for (var i = 1; i <= count; i++) {
		if ($('#chk' + i).is(':checked')) {
			country.push($("#chk" + i).val());
		}
	}
	if (country.length === 0) {
		alert("Please select the country name");
	} else {
		$.post("<?php echo base_url();?>head/location/changecountryStatus", {
			id: id,
			country: country
		}, function(data) {
...
function deleteCountry()
{
                     ...
                     for (var i=0;i<=count;i++)
                     {
                          if($('#chk'+i).is(':checked'))
                          {
                              countryid.push($("#chk"+i).val());                              
                          }
                     } 
                                          
                    $.post("<?php echo base_url();?>head/location/deleteCountry",{country_id:countryid},function(data){   
