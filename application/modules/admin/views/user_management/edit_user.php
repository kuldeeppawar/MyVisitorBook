			...
			<div class="col-md-12">
				<div class=" app-ticket-list">
					<div class="page-bar row">
						<div class="col-md-6">
							<h3 class="page-title"><?php getLocalkeyword('Edit Existing User');?></h3>
							<ul class="page-breadcrumb ">
								<li><a href="
										<?php echo base_url() ?>admin/dashboard"><?php getLocalkeyword('Home');?> </a> <i class="fa fa-circle"></i></li>
								<li><a href="
										<?php echo base_url() ?>admin/user_management"> <span><?php getLocalkeyword('User Management');?></span>>
								</a> <i class="fa fa-circle"></i></li>
								<li><span><?php getLocalkeyword('Edit User');?>Edit User</span></li>
							</ul>
						</div>
						<div class="col-md-6">
					...
					<div class="row">
						<div class="">
							<form role="form" action="<?php echo base_url() ?>admin/user_management/editUser" method="post">
								<div class="form-body mtop10">
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('Select Title');?> </label><select class="form-control" id="selUsertitle" name="selUsertitle" required>
												<option value="">Select Title</option>
												<option value="Mr" <?php if($resultUser[0]->sysu_title == "Mr"){ echo "selected"; } ?>>Mr</option>
												<option value="Miss" <?php if($resultUser[0]->sysu_title=="Miss"){ echo "selected"; } ?>>Miss</option>
												<option value="Shri" <?php if($resultUser[0]->sysu_title=="Shri"){ echo "selected"; } ?>>Shri</option>
											</select>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('First Name');?></label> <input type="text" class="form-control" placeholder="First name" name="txtUsername" id="txtUsername" value="<?php echo $resultUser[0]->sysu_name; ?>" required>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('Middle Name');?> </label> <input type="text" class="form-control" placeholder="Middle Name" id="txtMiddlename" name="txtMiddlename" value="<?php echo $resultUser[0]->sysu_mname; ?>" required>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('Last Name');?> </label> <input type="text" class="form-control" placeholder="Last Name" id="txtLastname" name="txtLastname" value="<?php echo $resultUser[0]->sysu_lname; ?>" required>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('Select branch');?><span class="mandatory">*</span>
											</label> <input type="hidden" name="txtUserid" value="
												<?php echo $resultUser[0]->sysu_id_pk; ?>"> <select class="form-control" name="selBranch" id-="selbranch" required="required">
												<?php for ($i = 0; $i < count($resultBranch); $i++)
													{ 
													
														if ($resultUser[0]->sysu_branchId_fk == $resultBranch[$i]->brn_id_pk)
														{	
													     ?>
													        <option value="<?php echo $resultBranch[$i]->brn_id_pk; ?>">
												         	<?php echo $resultBranch[$i]->brn_name; ?>
													        </option>
													<?php } ?>
												</select>
										</div>
									...
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('Select User Type');?><span class="mandatory">*</span>
											</label> <select class="form-control" name="selUsertype" id="selUsertype" required="required">
												<option value="">Select Usertype</option>
													<?php for ($i = 0; $i < count($resultUsertype); $i++) { ?>
													<option value="
														<?php echo $resultUsertype[$i]->utyp_id_pk; ?>"
													<?php
														
														if ($resultUser[0]->sysu_userTypeId_fk == $resultUsertype[$i]->utyp_id_pk)
														{
															echo "selected";
														}
														if ($resultUsertype[$i]->utyp_status == 0 && $resultUser[0]->sysu_userTypeId_fk != $resultUsertype[$i]->utyp_id_pk)
														{
															echo "disabled";
														}
														?>
												>
														<?php echo $resultUsertype[$i]->utyp_userType; ?>
													</option>
													<?php } ?>
												</select>
										</div>
									...
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('Mobile No.');?></label> <input type="text" value="<?php echo $this->encryption->decrypt($resultUser[0]->sysu_mobile); ?>" name="txtMobile" id="txtMobile" required="required" class="form-control" placeholder="Enter Mobile No. ">
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('Email Id');?> </label> <input type="text" value="<?php echo $this->encryption->decrypt($resultUser[0]->sysu_email); ?>" name="txtEmail" id="txtEmail" required="required" class="form-control" placeholder="Enter Email Id  "
												onblur="getValidateemail(this.value)"
											>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											...
											<div class="form-group">
												<textarea class="form-control" rows="1" name="txtAddress" id="txtAddress" required="required" placeholder="Type Address">
																	<?php echo $resultUser[0]->sysu_address?>
																</textarea>
											</div>
										</div>
									</div>
									<div class="col-md-3">
										<div class="form-group">
											<label><?php getLocalkeyword('Country');?> <span class="mandatory">*</span>
											</label> <select class="form-control" name="selCountry" id="selCountry" required="required" onchange="getState(this.value);">
												<option value="">Select Country</option>
																<?php for ($i = 0; $i < count($resultCountry); $i++) { ?>
																<option value="
																	<?php echo $resultCountry[$i]->cntr_id_pk; ?>"
													<?php
																	
																if ($resultUser[0]->sysu_countryId_fk == $resultCountry[$i]->cntr_id_pk)
																	{
																		echo "selected";
																	}
																	if ($resultCountry[$i]->cntr_status == 0 && $resultUser[0]->sysu_countryId_fk != $resultCountry[$i]->cntr_id_pk)
																	{
																		echo "disabled";
																	}
																	?>
										...
										<div class="form-group">
											<label><?php getLocalkeyword('State');?></label> <select class="form-control" onchange="getCity(this.value)" name="selState" id="selState" required="required">
												<option value="">Select State</option>
																<?php for ($i = 0; $i < count($resultState); $i++) { ?>
																<option value="
																	<?php echo $resultState[$i]->stat_id_pk; ?>"
													<?php
																	
																if ($resultUser[0]->sysu_stateId_fk == $resultState[$i]->stat_id_pk)
																	{
																		echo "selected";
																	}
																	if ($resultState[$i]->stat_status == 0 && $resultUser[0]->sysu_stateId_fk != $resultState[$i]->stat_id_pk)
																	{
																		echo "disabled";
																	}
																	?>
										...
										<div class="form-group">
											<label><?php getLocalkeyword('City');?></label> <select class="form-control" name="selCity" id="selCity" required="required">
												<option value="">Select City</option>
																...
																<option value="
																	<?php echo $resultCity[$i]->cty_id_pk; ?>"
													<?php
																	
															if ($resultUser[0]->sysu_cityId == $resultCity[$i]->cty_id_pk)
																	{
																		echo "selected";
																	}
																	if ($resultCity[$i]->cty_status == 0 && $resultUser[0]->sysu_cityId != $resultCity[$i]->cty_id_pk)
																	{
																		echo "disabled";
																	}
																	?>
								...
								<div class="col-md-12">
									<div class="modal-footer">
										...
										<a href="
															<?php echo base_url() ?>admin/user_management" class="btn btn-danger"><?php getLocalkeyword('Cancel');?> </a>
									</div>
								</div>
...
<script>function getCity(val) {
        $.post("
																							<?php echo base_url(); ?>admin/user_management/getCity", {val: val}, function (data) {
            $('#selCity').html(data);
        });
    }
    function getState(val) {
        $.post("
																							<?php echo base_url(); ?>admin/user_management/getState", {val: val}, function (data) {
            $('#selState').html(data);
        });
    }//---Function Used Validate Email--//function getValidateemail(value){  $.post("
																							<?php echo base_url(); ?>admin/user_management/getValidateemail",{email:value},function(data){                                   if(data==1)                   {                   	 alert("Email Allready Used");               	     var au="";               	     $("#txtEmail").val(au);               	                     }   });          }function validatePassword(val){		var password = document.getElementById("txtPassword"), confirm_password = document.getElementById("txtConfirmpassword");		if(password.value != confirm_password.value) {          //confirm_password.setCustomValidity("Passwords Don't Match");            alert("Password Mismatch");           password.value="";  		confirm_password.value="";	} else {				}	} 
																						</script>