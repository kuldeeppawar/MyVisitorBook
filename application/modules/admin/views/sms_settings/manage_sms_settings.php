			...
			<div class="col-md-12">
				<div class=" app-ticket-list">
					<div class="page-bar row">
						<div class="col-md-6">
							<h3 class="page-title"><?php getLocalkeyword('SMS Settings');?></h3>
							<ul class="page-breadcrumb ">
								...
								<li><span><?php getLocalkeyword('SMS Settings');?></span></li>
							</ul>
						</div>
						<div class="col-md-6">
									...
									<div class="col-md-5 pull-right">
										<div class="pull-right">
											<div style="float: left; margin-top: 8px; margin-right: 5px;">
												<?php
												if (getAdminAccessRights('mvbSetSmsSetEdit'))
												{
													?>
													<a class="margin_rgt hide_element"><i class="fa fa-pencil-square-o icon_custom" aria-hidden="true" title="Edit" onclick="editSmsSettings();"></i></a>
													<?php
												}
												
												if (getAdminAccessRights('mvbSetSmsSetDelete'))
												{
													?>
											...
											<?php
											if(checkPackageRights($this->session->userdata('client_id'),'sms_template_check'))
                                            {
														if (getAdminAccessRights('mvbSetSmsSetAdd'))
														{
											?>
	                                        ...
	                                        ?>
	                                        			<div class="btn-group" style="margin-left: 5px;">
			                                                   <font color="red">You have reached max limit to create sms template</font> 
			                                            </div>
	                                        <?php    	
	                                            }                                            
                                            ?>
																	...
																	<div class="row">
																		<div class="col-md-12">
																			<div class="modal-footer">
																				<a class="btn btn-primary" onclick="deleteSmsTemp();"><?php getLocalkeyword('Yes');?></a>
										...
										<table class="table table-striped table-bordered table-hover table-checkable order-column dataTable allchekbox" id="sample_1_2" role="grid" aria-describedby="sample_1_2_info">
											<thead>
												<tr>
													...
													<th><?php getLocalkeyword('Approve Date & Time');?></th>
												</tr>
											</thead>
											<tbody>
												<?php
												for($i=0;$i < count($result);$i ++)
												{
													?>
													<tr class="odd gradeX">
													<td><label> <input name="chk1" class="check_box" type="checkbox" data-ptag="sb1" id="chk1" value="<?php echo $result[$i]->str_id_pk;?>"> <span></span>
													</label></td>
													<td>
															<?php echo $result[$i]->str_template_name;?>
														</td>
													<td>
															<?php echo $result[$i]->str_description;?>
														</td>
													<td>
													...
													<td>
															<?php echo $result[$i]->admin_user_name;?>
														</td>
													<td>
				...
				<div class="portlet light bordered">
					...
					<div class="portlet-body" style="height: 650px; overflow: auto;" id="test-list">
						<ul class="list list-group All_list">
							...
							{
								?>
								<li class="list-group-item"><input type="checkbox" height="20" width="20" value="<?php echo $result[$j]->str_id_pk;?>" onclick="uncheck_checkbox_edit('<?php echo $result[$j]->str_id_pk;?>','<?php echo $j;?>');"
								id="temp_id_exists<?php echo $j;?>" name="temp_id_exists" class="checkbox1"
							/>
									<?php echo $result[$j]->str_template_name;?>
								</li>
								<?php
							}
							?>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="col-md-6" id="email_temp_dynamic">
							<form action="<?php echo base_url();?>admin/sms_settings/addSmsSettings" id="email_temp_form" name="sms_temp_form" method="POST">
								<div class="form-group">
									<label><?php getLocalkeyword('SMS Template Name');?> <span class="mandatory">*</span></label> <input type="text" class="form-control" placeholder="SMS Template Name" id="txttemplatename" name="txttemplatename" required>
								</div>
								<div class="form-group">
									<label><?php getLocalkeyword('Enter Content');?></label>
									<!--   <img src="../assets/pages/img/editor.png" /> -->
									<textarea class="form-control" rows="3" placeholder="SMS Body" style="height: 240px;" id="txtemailcontent" name="txtemailcontent" required></textarea>
								</div>
								<div class="form-group">
									<button type="submit" class="btn btn-primary" name="btnSubmit"><?php getLocalkeyword('Save');?></button>
									<a href="<?php echo base_url();?>admin/sms_settings" class="btn btn-danger"><?php getLocalkeyword('Cancel');?></a>
								</div>
							</form>
						</div>
						<div class="col-md-6">
							<div class="row">
								...
								<div class="col-md-12">
									<label><?php getLocalkeyword('Select Fields');?></label> <select class="form-control" id="selfields" name="selfields" onchange="getFieldsOptions(this.value);">
...
function getFieldsOptions(value) {
	...
	$.post("<?php echo base_url();?>admin/sms_settings/getFieldsOptions", {
...
function getTemplateDetails(email_temp_id) {
	$.post("<?php echo base_url();?>admin/sms_settings/getTemplateDetails", {
		temp_id: email_temp_id
	}, function(data) {
		$("#email_temp_dynamic").html(data);
	});
}
$("#sms_temp_form").validate({
	rules: {
		txttemplatename: {
			required: true
		},
...
function editSmsSettings() {
	...
	$.post("<?php echo base_url();?>admin/sms_settings/getRecordById", {
...
function deleteSmsTemp() {
	...
	$.post("<?php echo base_url();?>admin/sms_settings/deleteSmsTemp", {
		temp_id: tempid
	}, function(data) {
