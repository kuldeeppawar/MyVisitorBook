			...
			<div class="col-md-12">
				<div class=" app-ticket-list">
					<div class="page-bar row">
						<div class="col-md-6">
							<h3 class="page-title"><?php getLocalkeyword('Mo Registration');?></h3>
							<ul class="page-breadcrumb ">
								<li><a href="<?php echo base_url()?>admin/dashboard"><?php getLocalkeyword('Home');?></a> <i class="fa fa-circle"></i></li>
								<li><span><?php getLocalkeyword('Settings');?> </span></li>
								<li><span><?php getLocalkeyword('Mo Registration');?></span></li>
							</ul>
						</div>
									...
									<div class="col-md-5 pull-right">
										<div class="pull-right">
											<div style="float: left; margin-top: 8px; margin-right: 5px;">
											 <?php if(getAdminAccessRights('mvbmoEdit')){ ?>
												<a href="#"  onclick="getRecord();" class="margin_rgt p_element">
												 <i class="fa fa-pencil-square-o icon_custom" aria-hidden="true" title="Edit"></i>
												</a>
											 <?php }?>
											  <?php if(getAdminAccessRights('mvbmoDelete')){ ?>	
												 <a href="#" data-toggle="modal" id="sb1" data-target="#myModal_m" class="margin_rgt p_elementd">
												  <i class="fa fa-trash fa-2x" title="Delete" style="color: #828282;"></i></a>
												  <?php 
                                                    }
												  ?>
											</div>
											 <?php if(getAdminAccessRights('mvbmoAdd')){ ?>
											<div class="btn-group" style="margin-left: 5px;" data-toggle="modal" data-target="#myModal_new1">
												<a id="sample_editable_1_new" class="btn btn-primary"> <i class="fa fa-long-arrow-down"></i><?php getLocalkeyword('Add New MO');?> 
												</a>
											</div><?php 
												  }?>
											<ul class="nav navbar-nav pl0 pull-right" style="margin-left: 5px;">
												<li class="dropdown dropdown-user burger_menu">
												<a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown" data-hover="dropdown" data-close-others="true" style="margin-top: 0px;"><i class="fa fa-bars fa-2x" aria-hidden="true"></i>
												</a>
													<ul class="dropdown-menu dropdown-menu-default">
													    <?php if(getAdminAccessRights('mvbmoActive')){ ?>
														<li class="options" style="border-bottom: 1px solid #e7ecf1;"><a  onclick="chechCheckbox(1);" href="#" style="padding: 5px;"><?php getLocalkeyword('Active');?></a></li>
														<?php }?>
														<?php if(getAdminAccessRights('mvbmoDeactive')){ ?>
														<li class="options" style="border-bottom: 1px solid #e7ecf1;"><a  onclick="chechCheckbox(0);" href="#" style="padding: 5px;"><?php getLocalkeyword('Deactive');?></a></li>
													    <?php }?>
													</ul></li>
											</ul>
													...
													<div class="modal-content">
														<div class="modal-header">
															...
															<h2 class="modal-title text-left">
																<i class="fa fa-clone"></i> <?php getLocalkeyword('Add New Mobile No');?>
																</h3>
														
														</div>
														<div class="modal-body">
															<div class="portlet-body form">
																<form role="form" action="<?php echo base_url()?>admin/setting/addMo" method="post">
																	<div class="form-body">
																		<div class="row">
																			<div class="col-md-12 pull-left">
																				<div class="form-group">
																					<label><?php getLocalkeyword('Enter New MO');?> </label>
																					 <input type="number" step="any" min="0" class="form-control" placeholder="Enter New MO " name="txtMobile" required="required">
																				</div>
																...
																<form role="form">
																	<div class="form-body">
																		<div class="row">
																			<div class="col-md-12">
																				<div class="modal-footer">
																					<a href="#" class="btn btn-primary" onclick="getDelete();"><?php getLocalkeyword('Yes');?></a>
										...
										<table class="table table-striped table-bordered table-hover table-checkable order-column dataTable allchekbox" id="sample_1_2" role="grid" aria-describedby="sample_1_2_info">
											<thead>
												<tr>
													...
													<th><?php getLocalkeyword('Added Date and Time');?></th>
													<th><?php getLocalkeyword('Status');?></th>
												</tr>
											</thead>
											<tbody>
											   <?php for($i=0;$i<count($resultMobile);$i++)
											       {  
											       	   $j=$i+1;
											         ?>
													 <tr class="odd gradeX">
														<td><input name="chk<?php echo $j;?>" class="check_box" type="checkbox" data-ptag="sb1" id="chk<?php echo $j;?>" 
														           value="<?php echo $resultMobile[$i]->mreg_id_pk;?>"></td>
														<td><?php echo $resultMobile[$i]->mreg_no;?></td>
														<td><?php echo $resultMobile[$i]->mreg_createdDate;?></td>
														<td><?php if($resultMobile[$i]->mreg_status){ echo "Active";}else{ echo "Deactive";}?></td>
													</tr>
													<?php 
											         }
													?>
...
{
     var count=<?php echo count($resultMobile) ?>;
     var mobileId=[];
     ...
     for (var i=1;i<=count;i++)
     {
   	  if($('#chk'+i).is(':checked'))
   	  {
   		mobileId.push($("#chk"+i).val()); 
   	  }
      } 

     
     if (mobileId.length === 0) {
   	   alert ("Please Select The Mobile  Number");
   	      
   	}
     else
     {
         
          $.post("<?php echo base_url();?>admin/setting/changeMostatus",{id:id,mobileId:mobileId},function(data){
...
function getDelete()
{
     ...
     else
     {
          ...
          $.post("<?php echo base_url();?>admin/setting/deleteMo",{mobileId:mobileId},function(data){
     ...
     for (var i=1;i<=count;i++)
     {
   	  if($('#chk'+i).is(':checked'))
   	  {
   		 mobileId=$("#chk"+i).val(); 
   	      ...
   	      $.post("<?php echo base_url();?>admin/setting/getSpecificMo",{mobileId:mobileId},function(data){
