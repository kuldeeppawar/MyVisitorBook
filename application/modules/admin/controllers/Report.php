...
class Report extends CI_Controller
{
	...
	{
		$reportId=$this->input->post('val');
		$optCondition1=$this->input->post('val1');
		$optCondition2=$this->input->post('val2');
		$optCondition3=$this->input->post('val3');
		...
		{
			$fixedConditions="WHERE " . $clientfield ."=". $this->session->userdata('client_id') ." AND ".$branchfield . "=".$this->session->userdata('branch_id');
		} else if ($clientfield != "" && $branchfield == '')
		{
			$fixedConditions="WHERE " . $clientfield . "=".$this->session->userdata('client_id');
		} else
		{
	  	...
	  	for($i=0;$i < count($result);$i ++)
	    {
	    	...
	    	$tableData.='<tr class="odd gradeX">';
			    		...
			    		for($a=0;$a<count($fieldCount);$a++)
			    		{
			    			$tableData.="<td>".$result[$i]->$fieldCount[$a]."</td>";
			    	   ...
			    	   {	
				    		for($a=0;$a<count($numericCount);$a++)
				    		{
				    			$tableData.="<td>".$result[$i]->$numericCount[$a]."</td>";
				    		}
			    	   }	
			$tableData.='</tr>';
		}
