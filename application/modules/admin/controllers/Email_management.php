...
class Email_management extends CI_Controller
{
	...
	public function downloadCampCSV122()
	{
        ...
        public function cron_email_file() {
        
        // date comarison 
        $result_cron_email = $this->db->query("select * from tblmvbemailmanagement where em_email_schedule_status='1' and DATE_FORMAT('%Y-%m-%d',em_email_schedule_date)='".date('Y-m-d')."' ");
        $row_cron_email = $result_cron_email->result();
        if(count($row_cron_email) > 0 )
        {
            for($r=0;$r<count($row_cron_email);$r++)
            {
               $email_id =  $row_cron_email[$r]->em_id_pk;
               $email_schedule_date =  $row_cron_email[$r]->em_email_schedule_date;
               if($email_schedule_date!='0000-00-00 00:00:00')
               {
                    $result_cron_email_user = $this->db->query("select * from tblmvbsendemailusers where seu_em_id_fk='".$email_id."' ");
                    $row_cron_email_user = $result_cron_email_user->result();
                    if(count($row_cron_email_user) > 0 )
                    {
                        for($su=0;$su<count($row_cron_email_user);$su++)
                        {
                           $txtMessage =  $row_cron_email_user[$su]->seu_message_details;
                           $user_id =  $row_cron_email_user[$su]->seu_visitor_id;
                           $branch_id =  $row_cron_email_user[$su]->seu_branchId_fk;
                            // check balance of              
                            ...
                            if($check_msg_length<=160)
                             ...
                             {
                                    $email_count=round($check_msg_length/160);
                             }
                             //branch wise check sms balance here 

                            $check_email_balance=$this->email_management_model->getBranchBalance($branch_id);
                            if($check_email_balance > 0 )
                            {
                                // send email functionality
                                $userMessageStr=$this->EmailString($txtMessage,$user_id);
				$EmailId=$this->getUserEmailId($user_id);
				$this->sendEMAIL($EmailId->vis_email,$userMessageStr);
                                ...
                                $this->db->query("update set seu_emailstatus = '1' from tblmvbsendemailusers where ssu_visitor_id ='".$user_id."' ");
                                //log ki entry 
                                
                                //update sms count in branch 
                                
                                $this->db->query("update set brn_emailCredit = brn_emailCredit - '1' from tblmvbbranches where brn_id_pk ='".$branch_id."' ");
                               
                                $cmapainUserLogs=array('seu_em_id_fk'=>$email_id,
                                                        'seu_visitor_id'=>$user_id,
                                                        'seu_branchId_fk'=>$branch_id,    
                                                        'seu_message_details'=>$userMessageStr,
                                                        'seu_cDate'=>date('Y-m-d H:i:s'),    
                                                        'seu_emailstatus'=>1);

                                $this->db->insert('tblmvbsendemailuserslogs',$cmapainUserLogs);
                            }
                            else
                            {
                                // log sms failed
                                   $cmapainUserLogs=array('seu_em_id_fk'=>$email_id,
                                                            ...
                                                            'seu_message_details'=>$userMessageStr,
                                                            'seu_cDate'=>date('Y-m-d H:i:s'),
                                                            'seu_failed_reason'=>'insuficient_balance',    
                                                            'seu_emailstatus'=>0);
