...
class Branch_model extends CI_Model
{
	...
	public function addBranch()
	{
		...
		if (count($parentBranch) > 0)
		{
			...
			$data=array('brn_clientId_fk'=>$clientId,
				'brn_name'=>$branchName,
				...
				'brn_smsCredit_deduct'=>$branchsmsCredit,
				'brn_emailCredit_deduct'=>$branchEmailcredit,
				'brn_validity'=>90,
				'brn_isChildBranch'=>1,
		...
		$data=array('brcr_brnId_fk'=>$insert_id,
		...
		if ($brancholdsms > $branchsmsCredit || $brancholdEmail > $branchEmailcredit)
		  {
		  	...
		  	if($brancholdEmail > $branchEmailcredit)
		  	{
		   	...
		   	$data=array('brcr_brnId_fk'=>$branchId,
		   			    'brcr_clntId_fk'=>$clientId,
		...
		else if($brancholdsms < $branchsmsCredit || $brancholdEmail < $branchEmailcredit)
		  {
		  	...
		  	if($brancholdEmail > $branchEmailcredit)
		  	{
		   	...
		   	$this->db->query("UPDATE tblmvbclients SET clnt_totalSmsCredit=clnt_totalSmsCredit-'" . $clientSMSnew . "',clnt_totalEmailCredit=clnt_totalEmailCredit-'" . $clientEmailnew . "'  WHERE clnt_id_pk='" . $clientId . "' ");
	...
	public function addRefillsms()
	{
	   ...
	   //this for update branch		sms
			$this->db->query("UPDATE tblmvbbranches SET brn_smsCredit=brn_smsCredit+'" . $branchsmsCredit . "',brn_smsCredit_deduct=brn_smsCredit_deduct+'".$branchsmsCredit."'
		...
		$this->db->query("UPDATE tblmvbclients SET clnt_totalSmsCredit=clnt_totalSmsCredit-'" . $branchsmsCredit . "' WHERE clnt_id_pk='" . $clientId . "' ");
	...
	public function addRefillemail()
	{
		...
		//this for update branch		sms
		$this->db->query("UPDATE tblmvbbranches SET brn_emailCredit=brn_emailCredit+'" . $branchemailCredit . "',brn_emailCredit_deduct=brn_emailCredit_deduct+'" . $branchemailCredit . "' WHERE brn_id_pk='" . $branchId . "' ");
   ...
   public  function mergeBranch($branchId,$mainbranchId)
   {
   	     ...
   	     $data=array('brmr_activebrId_fk'=>$mainbranchId,
