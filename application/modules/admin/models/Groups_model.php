...
class Groups_model extends CI_Model
{
	...
	public function getAllGroups($operation)
	{
		$branchId=$this->session->userdata('branch_id');
        $adminId=$this->session->userdata('admin_admin_id');

        $query = "SELECT  tblmvbgroups.grp_id_pk,tblmvbgroups.grp_name,tblmvbgroups.grp_status,tblmvbgroups.grp_createdDate,
				                 ...
				                 tblmvbgroups.grp_branchId_fk='".$branchId."' and tblmvbgroups.grp_status!='2' GROUP BY tblmvbgroups.grp_id_pk";

        if($operation == '')
        {
            $query1 = '';
        }
        elseif ($operation == 'getRecentViewGroup')
        {
            $query1 = ' ORDER BY tblmvbgroups.grp_modifiedDate DESC';
        }
        elseif ($operation == 'getMostViewGroup')
        {
            $query1 = ' ORDER BY tblmvbgroups.grp_view_count DESC';
        }
        elseif ($operation == 'getNotUpdatedRecently')
        {
            $d2 = date('Y-m-d', strtotime('today - 30 days'));
            $query1 = ' and tblmvbgroups.grp_modifiedDate < '.$d2.'';
        }
...
//       $result=$this->db->query("DELETE FROM `tblmvbgroupvisitors` WHERE gv_grpId_fk='".$groupId."' and gv_visitorId_fk='".$visitorId."'");
    ...
    public function createGroup($groupId)
    {
        ...
        $result=$this->db->query("select * From tblmvbgroups LEFT JOIN tblmvbgroupvisitors on tblmvbgroupvisitors.gv_grpId_fk=tblmvbgroups.grp_id_pk WHERE tblmvbgroups.grp_id_pk='".$groupId."'");
...
//        print_r($final_result); die;
        ...
        $data=array('grp_branchId_fk'=>$branchId,
            'grp_name'=>$final_result[0]->grp_name.'_copy',
        ...
        $insert_id_group=$this->db->insert_id();


        for($i=0;$i<count($final_result);$i++)
        {
            $data=array('gv_grpId_fk'=>$insert_id_group,
                'gv_visitorId_fk'=>$final_result[$i]->gv_visitorId_fk,
                'gv_addedDate'=>$date,
                'gv_addedBy'=>$adminId,
                'gv_status'=>1
            );
            $this->db->insert('tblmvbgroupvisitors',$data);
        }
...
//							}
                            ...
                            $visitorIdArray = [];
                            $visitorMobileNo=$importData[2]; //Mobile Number

                            $query=$this->db->query("SELECT vis_id_pk,vis_firstName,vis_lastName FROM `tblmvbvisitors` WHERE vis_mobile='" . $visitorMobileNo . "' ");
                            $is_Present = $query->result();

                            if(count($is_Present) >= '1')
                            {
                                array_push(array_push($visitorIdArray, $is_Present[0]->vis_id_pk, $is_Present[0]->vis_firstName, $visitorMobileNo));
                            }
                            else
                            {
                                $data=array('vis_clientId_fk'=>$clientId,
                                    'vis_branchId_fk'=>$branchId,
                                    'vis_firstName'=>$importData[1],
                                    'vis_lastName'=>$importData[1],
                                    'vis_mobile'=>$visitorMobileNo,
                                    'vis_status'=>1
                                );
                                $this->db->insert('tblmvbvisitors',$data);

                                $insert_id_visitor=$this->db->insert_id();

                                array_push(array_push($visitorIdArray, $insert_id_visitor,$importData[1],$visitorMobileNo));

                            }

                            $resultVisit=$this->getVisitordetails($visitorIdArray[0]);
                            if(count($resultVisit)>0 )
                            {
                                ...
                                {
                                    $datagroup=array('gv_grpId_fk'=>$insert_id,
                                        'gv_visitorId_fk'=>$visitorIdArray[0],
                                        'gv_addedDate'=>$date,
                                        'gv_addedBy'=>$adminId,
                                        'gv_status'=>1
                                    );
