...
class Visitor_model extends CI_Model
{
  ...
  public function getVisitorgroup($id)
  {
	...
	public function editVisitor()
	{
		...
		setAActivityLogs('Transaction_activity','AAddvisitorform_Edit',"Update Data For Visitor id :- ".$insert_id." -> Field data update for :- ".$idlist."  field Values:-".$otherdata);
	...
	{
			$result=$this->db->query("SELECT ('Revisited at ') as history_title,rv_purpose as history_description,DATE_FORMAT(rv_date,'%Y-%m-%d %H:%i %p') as action_date FROM tblmvbrevisitors WHERE 
			                      rv_visitors_id='".$visitor_id."'
			                          UNION
			                          SELECT ('Created at ') as history_title,('Visitor created') as history_description,DATE_FORMAT(vis_createdDate,'%Y-%m-%d %H:%i %p') as action_date FROM tblmvbvisitors
 									  WHERE vis_id_pk='".$visitor_id."'
 									  UNION
 									  SELECT ('Updated at ') as history_title,('Visitor data updated') as history_description,DATE_FORMAT(atl_activity_time,'%Y-%m-%d %H:%i %p') as action_date FROM tblmvbatransactionlogs
 									  WHERE atl_type='AAddvisitorform_Edit' and atl_activity_details LIKE '%Visitor id :- ".$visitor_id."%'
                                      UNION
 									  SELECT ('Email sent at ') as history_title,('Email sent') as history_description,DATE_FORMAT(seu_cDate,'%Y-%m-%d %H:%i %p') as action_date FROM tblmvbsendemailuserslogs
 									  WHERE seu_visitor_id='".$visitor_id."'
                                      UNION
 									  SELECT ('SMS sent at ') as history_title,('SMS sent') as history_description,DATE_FORMAT(ssul_cDate,'%Y-%m-%d %H:%i %p') as action_date FROM tblmvbsendsmsuserslogs
